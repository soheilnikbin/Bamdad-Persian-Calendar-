...
public class DateHelper {
    ...
    public static String getDateTimeShort(Context mContext, Long date) {
        ...
        CivilDate civilDateintro = new CivilDate(c.get(Calendar.YEAR)
                                                ,c.get(Calendar.MONTH) + 1
                                                ,c.get(Calendar.DAY_OF_MONTH));

        persianDate = DateConverter.civilToPersian(civilDateintro);
        Utils utils = Utils.getInstance(mContext);

        String hour = ConvertPersianCal.toPersianNumber(Integer.toString(c.get(Calendar.HOUR)));
        String minute =  ConvertPersianCal.toPersianNumber(Integer.toString(c.get(Calendar.MINUTE)));
        return utils.shape(utils.dateToString(persianDate))
                + " " + hour + ":" + minute;
    }
...
//		return DateUtils.formatDateTime(mContext, time, DateUtils.FORMAT_SHOW_TIME);
    ...
    public static String getNoteReminderText(long reminder) {
        return OmniNotes.getAppContext().getString(R.string.alarm_set_on) + " " + getDateTimeShort(OmniNotes.getAppContext(), reminder);
    }
