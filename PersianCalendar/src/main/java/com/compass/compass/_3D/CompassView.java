...
package com.compass.compass._3D;
...
public class CompassView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float cy = h + (w / 2.5f);
        float cp = (w * 2) / 3;
        cy -= ((Main) getContext()).getBottomMargin();
        h -= ((Main) getContext()).getBottomMargin();

        mPaint.setStyle(Style.FILL);
        mPaint.setColor(0xFF263238);
        ...
        canvas.drawText(Utils.toArabicNrs(ConvertPersianCal.toPersianNumber(Math.round(mX)+"")) + "°", w / 2, h * 0.9f, mPaint);

        mPaint.setTextSize(w / 12);
        mPaint.setTextAlign(Align.RIGHT);
        canvas.drawText(Utils.toArabicNrs(ConvertPersianCal.toPersianNumber(Math.round(mqDist)+"")) + "km", w * 0.45f, h * 0.98f, mPaint);

        mPaint.setTextAlign(Align.LEFT);
        canvas.drawText(Utils.toArabicNrs(ConvertPersianCal.toPersianNumber((int)Math.round(mqAngle)+"")) + "°", w * 0.55f, h * 0.98f, mPaint);
        ...
        if (mqAngle != 0) {
            ...
            canvas.drawText("شمال", w / 2, h / 2, mPaint);
